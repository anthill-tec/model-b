...
- **`~/.claude/scripts/worktree-flow.py`** — parallel-CR isolation: `start`/`status`/`sync`/`finish`/`abort`. Git-based, **stack-agnostic** — Java CRs use it unchanged. `finish` = `merge --no-ff` → `worktree remove` → `branch -d`; run from the integration tree, not inside the worktree (`ExitWorktree` first); atomic merge lock serializes parallel finishes. Does NOT replace `git flow feature start` for sequential single-CR work.
  - **ChangeSet scheduling loop is stack-agnostic — a Java track gets its next CR exactly like Rust.** The `next → start → finish` loop (+ `cs`/`status`/`reconcile`) and HOLD/DRAINED semantics are documented in `orchestration-track.md`; the `worktree-flow` command set + ChangeSet DB mechanics are in `rust-orchestration.md` (the DB is gitignored, `depends_on`-driven readiness).
