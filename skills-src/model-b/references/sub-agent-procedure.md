...
- **If spawned in a worktree, that worktree is your ONLY writable root.** Establish it FIRST: `git rev-parse --show-toplevel` from cwd — it MUST equal `…/.claude/worktrees/<cr>/`, not the main tree. (Not spawned in a worktree — e.g. a non-Model-B project? This section does not apply; write normally.)
...
- **This is HARD-ENFORCED.** A `PreToolUse` hook (`block-write-outside-worktree`) DENIES any `Write`/`Edit`/`NotebookEdit` whose target is outside your worktree root. If you hit that block, your path was wrong — fix it to stay inside the worktree; do NOT try to bypass it (no `dangerouslyDisableSandbox`, no shell workaround). The `ALLOW_WRITE_OUTSIDE_WORKTREE=1` escape hatch is for ORCHESTRATORS only — never a sub-agent.
