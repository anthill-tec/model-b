...
- The MOMENT `worktree-flow start` creates the worktree, **enter it with `modelb_worktree_enter`** and the path `start` prints. The `block-write-outside-worktree` hook then blocks your writes outside it, and every agent you dispatch naming the CR runs rooted in it under the same hook. Your session cwd stays the main tree (it is fixed at launch) — never rely on per-command `cd` or an empty-var `git -C ""`, which fall back to develop.
