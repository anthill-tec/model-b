...
**Authority:** the Model-B-owned bundled skill `crucible-report-vscode`
(`skills-src/crucible-report-vscode/`, deployed by the modelb-axi installer)
— the documented interim inline urllib ingest
procedure (parse JUnit → `POST /api/v2/runs/parsed`, build failures →
`/api/v2/runs/compile`) lives THERE. Do not build a
