...
- Baseline for `python3 -m unittest discover -s tests -t .`: **361 tests, 6 failures, 12 skips** (measured 2026-09-21 on `feature/CR-MDB-017-client-role-contract-sync`; the earlier 270/7F/12S and 240/1F/12S figures are superseded). **All six failures are the `chezmoi diff` gates CR-MDB-021 retires** — `test_core_split.py::CoreSplitS5ShimRemovalAndArchiveTest`, `test_cr_authoring_skill.py::CrAuthoringSkillS3Test`, `test_crucible_skill.py::CrucibleSkillS4Test`, `test_git_chezmoi_skills.py::DeletionsS4Test`, `test_memory_model.py::MemoryModelS4Test` and `test_model_b_skill.py::ModelBSkillS4Test`, each `test_s<n>_chezmoi_diff_clean_on_cr_touched_paths`. They compare the user's dotfile source to the user's home and flap on drift that is not a Model B property. The seventh failure is GONE, cleared by CR-MDB-017: `test_crucible_skill.py::CrucibleSkillCRMDB011Test::test_ac7_repo_agents_md_no_longer_claims_workflow_cycle_id_injection` was a bare whole-file substring count that forbade this file from naming the retired term it forbids, so describing the gate tripped it. It is re-pointed at the CLAIM (no sentence may say the wrapper injects the retired per-run cycle-id variable), backed by a detector-bites fixture and a positive product-surface sweep over `modelb_axi/`, `scripts/`, `generator/` and `skills-src/` — so CR-MDB-032 §S3 no longer needs to scope it. Two skips are permanently dead tests waiting for a retired origin directory (CR-MDB-032 §S1). Compare against this baseline rather than expecting a fully green run; `audits/2026-09-21-codebase-review-tests.md` has the per-test diagnosis.
