...
2. **Register with Crucible** via the stable stack client (NOT inline curl/python):
   ```bash
   python3 ~/.crucible/clients/python-crucible.py register --agent YOUR_AGENT_ID --role GREEN --cycle YOUR_CYCLE_ID
   ```
...
- The only standard tools to reach for directly: **Read** (a file you'll `Edit`), **Glob**, **ctx_shell** (crucible client + git).
