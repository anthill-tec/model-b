...
effort: high
maxTurns: 500
tools: read, grep, find, ls, ctx_shell, ctx_read, ctx_grep, ctx_glob, ctx_find, ctx_ls, ctx_search, ctx_tree, lean_ctx
permission:
  read: allow
  grep: allow
  find: allow
...
1. **Register with Crucible** via the stable stack client:
   ```bash
   python3 ~/.crucible/clients/python-crucible.py register --agent YOUR_AGENT_ID --role VERIFY --cycle YOUR_CYCLE_ID
   ```
