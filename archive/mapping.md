...
- **Patterns.** `<name>` stands for one path component; `{a,b}` for one of the listed names. A
  ...
  the file's original location; its archived copy is under `archive/wave1..3/` or
  `archive/contracts/`.
...
| `crucible:docs/research/DN-model-b-language.md` | moved | `docs/research/DN-model-b-language.md` | Crucible owns the LOCKED ontology; Model B holds a frozen import (origin `a9a8f57`) | 2616ad9 |
...
| `~/.claude/memory/{java-,}{coding-standards,testing-practices,modern-syntax,maven-best-practices,quarkus-patterns}.md` | moved | `skills-src/memory-templates/` | Model B; the five Java references (originals `java-coding-standards`, `java-testing-practices`, `java-modern-syntax`, `maven-best-practices`, `quarkus-patterns`; all now `java-`-prefixed); PRD §D5 amended — no global memory tier | 2616ad9, CR-MDB-035 |
...
| `~/.claude/agents/{arduino,bun,python,quarkus}-<role>-agent.md` | absorbed | `generator/templates/` | Model B; role templates + `generator/stacks/*.toml`, rendered to `generator/agents/` and per project into `.pi/agents/` | CR-MDB-008, CR-MDB-014, CR-MDB-025 |
...
| `~/.claude/agents/` | external | `<project>/.pi/agents/` (Model B stack × role definitions, rendered by `modelb-axi init`/`agents`) | Model B renders per project; the rest of the set (electronics, `inbox-analyst`, vscode) is the user's (DN-multi-harness §D17) | CR-MDB-025, CR-MDB-024 |
...
| `.claude/worktrees/<cr>` | moved | `contracts/worktree-layout.md` | Model B; worktrees now live at `.worktrees/<cr>` (gitignored); the contract names its six consumers | CR-MDB-031 |
...
| `tests/test_<topic>.py` | absorbed | `tests/_helpers.py` | Model B; module-level helper copies (18 families) live once | CR-MDB-032 |
