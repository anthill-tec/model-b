...
- Every orchestrator session is BRACKETED by two skills: **`/bootstrap <role>`** at start (register + relaunch the Sandesh notifier, recover carried todos, load queue/hold per role) and **`/shutdown`** at end (finish the active step, drain todos + merge the active CR, leave no dangling/uncommitted work, ack, then kill the notifier LAST).
...
- **The notifier-kill at shutdown is the SINGLE override of the relaunch-on-exit prime directive** — everywhere else a watcher that exits is relaunched the same turn (never left dead); ONLY at a confirmed shutdown's final step is it killed and not relaunched. Keep it alive through the whole teardown (to receive acks / a late emergency-stop).
