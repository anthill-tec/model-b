...
def _emit_opencode(
    ...
    for instance in instances:
        ...
        lines += [
            ...
            f'  const result = spawnSync("{script_path}", {{ input: JSON.stringify(payload) }});',
...
#: pi extension event names (DN §2 roster addendum) per universal event.
_PI_EVENTS = {
...
def _emit_pi(
    ...
    for instance in instances:
        script_path = str(scripts_root / instance["command"])
        pi_event = _PI_EVENTS[instance["event"]]
        text = (
            "// Generated by modelb_axi hooks compiler (CR-MDB-015 §S4) — do not edit.\n"
