...
### §S2 — `~/.claude/skills/crucible/SKILL.md` sync
...
### §S3 — `references/{rust,java,bun,python,vscode}.md` → thin routers
...
### §S6 — chezmoi round-trip
Skill edits mirrored via `chezmoi add` with the no-auto config + manual source commit; never `apply`, never push.

## Acceptance criteria

- [ ] AC1: zero `WORKFLOW_CYCLE_ID` occurrences under `~/.claude/skills/crucible/` (SKILL.md + all references) and in `contracts/crucible-envelope.md`.
...
- [ ] AC6: all five `references/{stack}.md` paths still exist (consumer constraint — 22 agents + 2 refactorer skills).
- [ ] AC7: repo `AGENTS.md` wrapper sentence no longer claims `WORKFLOW_CYCLE_ID` injection.
- [ ] AC8: chezmoi source updated for every `~/.claude` file touched; `chezmoi diff` clean on those paths (no-auto config; no apply/push).
