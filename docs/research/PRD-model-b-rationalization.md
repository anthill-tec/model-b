...
- **The skill carries the UNIVERSAL Model B conventions, parameterized per project (user directive 2026-07-20)** — every project instantiates them from its canonical token + acronym:
  ...
  3a. *Wave-boundary gate (user rule 2026-07-20):* the no-mistakes gate runs at every wave boundary and ingests as gate evidence. It requires a REMOTE — when the project has none, the ORCHESTRATOR ESCALATES to the user for permission to set one up; a LOCAL substitute gate (full regression + grep gates, ingested via `gate-report`) is used ONLY on the user's explicit denial.
