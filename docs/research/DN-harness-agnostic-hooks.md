...
- **Roster addendum (researched 2026-07-22, CR-MDB-015 §S4 step 0; primary docs: pi.dev/docs/latest/extensions · hermes-agent.nousresearch.com/docs/reference/cli-commands + /docs/user-guide/configuration):**
  - **pi (pi.dev / earendil-works):** FULL project-level support via TS extensions (`.pi/extensions/*.ts`, loaded after a project-TRUST prompt recorded in `~/.pi/agent/trust.json`). Events cover the universal set and more: `session_start`, `tool_call` (mutable input; return `{block:true, reason}` blocks), `tool_result`, `before_agent_start`, etc. Extensions spawn subprocesses via `pi.exec` — the DN §3(c) spawn-shim strategy applies directly (opencode-class emitter). Fail-closed IS honorable (shim blocks on spawn failure). Trust churn: the project-trust prompt is the re-confirm step (documented, accepted). **Event-map citations (VERIFY B2 follow-up, same primary doc):** the universal set maps to pi as `session-start`→`session_start`, `pre-tool-use`→`tool_call` (mutable input; `{block:true}`), `post-tool-use`→`tool_result`, `turn-stop`→`turn_end` (per-turn lifecycle events documented), `prompt-submit`→`input` ("when user sends input; can intercept or transform"). `pre-compact` has NO documented pi counterpart — DECLARED GAP: not emitted for pi, noted in the compiler report (same declared-degradation idiom as Hermes).
  - **Hermes (NousResearch hermes-agent):** shell-script hooks EXIST but are USER-SCOPE ONLY (`~/.hermes/config.yaml`; first-use consent allowlist `~/.hermes/shell-hooks-allowlist.json`; `hermes hooks list/test/doctor/revoke`); stdin-JSON payloads + per-hook timeout confirmed, but the BLOCKING contract (exit-code semantics) is under-documented and NO project-level hook declaration exists (project `./.hermes/plugins/` is a python plugin API behind an env opt-in, not shell hooks). v1 verdict: DECLARED DEGRADATION — the compiler emits an ADVISORY user-scope config snippet (manual adoption; consent allowlist forces human approval anyway) + a degradation report line; security-class fail-closed hooks are REFUSED for Hermes (blocking semantics unverifiable). Revisit when project-level declaration or a documented blocking contract ships.
